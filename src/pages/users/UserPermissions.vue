          ...
          >
            ...
            <q-card-actions align="right">
              <q-btn
                ...
                size="sm"
                flat
                @click="addGroupToUser(group, closeDialog)"
              />
            </q-card-actions>
          </q-card>
        </q-card-section>
...
<script lang="ts">
// TODO: break this component up into smaller groups
import {
  defineComponent, PropType, ref, computed,
} from 'vue';
import { getPermissionGroups } from 'src/api/permissions';
import { userScopeUpdate } from 'src/api/users';
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    async function addGroupToUser(group: PermissionGroup, closeDialog: () => void) {
      const userCopy = JSON.parse(JSON.stringify(props.userDetail));
      userCopy.scopes.push(group.name);
      const res = await userScopeUpdate(props.userDetail.id, userCopy);
    ...
    async function removedGroupFromUser(group: PermissionGroup) {
      const userCopy = JSON.parse(JSON.stringify(props.userDetail));
      const idx = userCopy.scopes.indexOf(group.name);
      if (idx !== -1) {
        userCopy.scopes.splice(idx, 1);
      }
      const res = await userScopeUpdate(props.userDetail.id, userCopy);
      if (res && res.data && res.status === 200) {
        emit('updated', res.data);
